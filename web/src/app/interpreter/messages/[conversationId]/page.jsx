    ...
    const loadMeta = async () => {
      try {
        ...
        if (convDoc.exists() && user) {
          ...
          if (otherId) setOtherUser(data.participantsData?.[otherId] || { displayName: 'Client' });
          if (data.contactRevealed) {
            setContactRevealed(true);
            const userDoc = await getDoc(doc(db, 'users', otherId));
            if (userDoc.exists()) setContactInfo(userDoc.data());
          }
        }
      } catch {}
    };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-3xl">
      ...
      <div className="bg-white border-b border-[#e7e8e9] rounded-t-2xl px-4 py-3 flex items-center gap-3 shadow-card">
        ...
        <div>
          <p className="text-sm font-semibold text-[#191c1d]">{otherUser?.displayName || 'Client'}</p>
          <p className="text-xs text-[#737686]">Client</p>
        </div>
      </div>

      {/* Contact revealed banner */}
      {contactRevealed && contactInfo && (
        <div className="bg-green-50 border-b border-green-200 px-4 py-2.5 flex items-center gap-4">
          <span className="text-xs font-semibold text-green-800">Contact révélé</span>
          {contactInfo.phone && (
            <a href={`tel:${contactInfo.phone}`} className="flex items-center gap-1.5 text-xs text-green-700 hover:text-green-900">
              <Phone size={12} /> {contactInfo.phone}
            </a>
          )}
          {contactInfo.email && (
            <a href={`mailto:${contactInfo.email}`} className="flex items-center gap-1.5 text-xs text-green-700 hover:text-green-900">
      ...
      <div className="flex-1 overflow-y-auto bg-[#f8f9fa] px-4 py-4 space-y-1">
        {loading ? (
          ...
          <div className="text-center py-12"><p className="text-sm text-[#737686]">Démarrez la conversation...</p></div>
        ) : (
          messages.map((msg, i) => {
              ...
              <div key={msg.id}>
                ...
                {isSystem ? (
                  ...
                  <div className={`flex items-end gap-2 mb-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
                    {!isMe && <Avatar name={otherUser?.displayName || '?'} size="xs" />}
                    <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${isMe ? 'bg-[#004ac6] text-white rounded-br-sm' : 'bg-white text-[#191c1d] shadow-card rounded-bl-sm'}`}>
                      <p className="leading-relaxed">{msg.text}</p>
                      <p className={`text-[10px] mt-1 ${isMe ? 'text-blue-200 text-right' : 'text-[#737686]'}`}>{formatTime(msg.timestamp)}</p>
                    </div>
                  </div>
                )}
              </div>
