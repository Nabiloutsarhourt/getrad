  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#434655] mt-1">Vos conversations avec les prestataires</p>
      </div>
      ...
      {conversations.length === 0 ? (
        <div className="bg-white rounded-2xl p-10 text-center shadow-card">
          ...
          <p className="text-xs text-[#737686] mt-1">Les conversations démarrent après une réservation ou acceptation de mission.</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl shadow-card divide-y divide-[#f3f4f5] overflow-hidden">
          {conversations.map(conv => {
            ...
            const unread = otherId ? (conv.unreadCount?.[user?.uid] || 0) : 0;
            ...
            return (
              ...
              >
                <div className="relative">
                  ...
                  {unread > 0 && (
                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#004ac6] text-white text-xs font-bold rounded-full flex items-center justify-center">
                      {unread}
                    </span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-sm ${unread > 0 ? 'font-bold text-[#191c1d]' : 'font-medium text-[#191c1d]'} truncate`}>
                      {otherData?.displayName || 'Interlocuteur'}
                    </p>
