// Service administration — GETRAD
import {
  collection, query, getDocs, getDoc,
  doc, setDoc, updateDoc, orderBy,
...
export const getDashboardStats = async () => {
  try {
    const [usersSnap, missionsSnap, subsSnap, verifsSnap, availReqSnap] = await Promise.all([
      ...
      getDocs(collection(db, 'availabilityRequests')).catch(() => ({ docs: [] })),
    ]);
    ...
    const availReqs = availReqSnap.docs.map(d => d.data());
    ...
    return {
      ...
      stats: {
        ...
        availabilityRequests: {
          total:    availReqs.length,
          pending:  availReqs.filter(r => r.status === 'pending').length,
          accepted: availReqs.filter(r => r.status === 'accepted').length,
          refused:  availReqs.filter(r => r.status === 'refused').length,
          expired:  availReqs.filter(r => r.status === 'expired').length,
        },
      },
    };
  } catch (error) {
